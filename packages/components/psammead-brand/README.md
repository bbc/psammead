...
When using `Brand` on at the top pages, you should ensure that `borderBottom` prop is set to true. Also when using brand on the footer you should set `borderTop` to true. This ensures that in high contrast mode the start/end of the `Brand` component is clearly seen.

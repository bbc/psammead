...
test('parse', function () {
    var tests = 'जनवरी जन._फ़रवरी फ़र._मार्च मार्च_अप्रैल अप्रै._मई मई_जून जून_जुलाई जुल._अगस्त अग._सितंबर सित._अक्टूबर अक्टू._नवंबर नव._दिसंबर दिस.'.split('_'), i;
...
test('format', function () {
    var a = [
        ['dddd, Do MMMM YYYY, a h:mm:ss बजे', 'रविवार, 14 फ़रवरी 2010, दोपहर 3:25:50 बजे'],
...
test('format month', function () {
    var expected = 'जनवरी जन._फ़रवरी फ़र._मार्च मार्च_अप्रैल अप्रै._मई मई_जून जून_जुलाई जुल._अगस्त अग._सितंबर सित._अक्टूबर अक्टू._नवंबर नव._दिसंबर दिस.'.split('_'), i;
...
test('from', function () {
    ...
    assert.equal(start.from(moment([2007, 1, 28]).add({ s: 90 }), true), '2 मिनट', '90 seconds = 2 minutes');
    assert.equal(start.from(moment([2007, 1, 28]).add({ m: 44 }), true), '44 मिनट', '44 minutes = 44 minutes');
    ...
    assert.equal(start.from(moment([2007, 1, 28]).add({ m: 90 }), true), '2 घंटे', '90 minutes = 2 hours');
    assert.equal(start.from(moment([2007, 1, 28]).add({ h: 5 }), true), '5 घंटे', '5 hours = 5 hours');
    assert.equal(start.from(moment([2007, 1, 28]).add({ h: 21 }), true), '21 घंटे', '21 hours = 21 hours');
    ...
    assert.equal(start.from(moment([2007, 1, 28]).add({ d: 46 }), true), '2 महीने', '46 days = 2 months');
    assert.equal(start.from(moment([2007, 1, 28]).add({ d: 74 }), true), '2 महीने', '75 days = 2 months');
    assert.equal(start.from(moment([2007, 1, 28]).add({ d: 76 }), true), '3 महीने', '76 days = 3 months');
    assert.equal(start.from(moment([2007, 1, 28]).add({ M: 1 }), true), 'एक महीने', '1 month = a month');
    assert.equal(start.from(moment([2007, 1, 28]).add({ M: 5 }), true), '5 महीने', '5 months = 5 months');
    assert.equal(start.from(moment([2007, 1, 28]).add({ d: 345 }), true), 'एक वर्ष', '345 days = a year');
    assert.equal(start.from(moment([2007, 1, 28]).add({ d: 548 }), true), '2 वर्ष', '548 days = 2 years');
    assert.equal(start.from(moment([2007, 1, 28]).add({ y: 1 }), true), 'एक वर्ष', '1 year = a year');
    assert.equal(start.from(moment([2007, 1, 28]).add({ y: 5 }), true), '5 वर्ष', '5 years = 5 years');
});
...
test('calendar day', function () {
    ...
    assert.equal(moment(a).calendar(), 'आज दोपहर 12:00 बजे', 'today at the same time');
    assert.equal(moment(a).add({ m: 25 }).calendar(), 'आज दोपहर 12:25 बजे', 'Now plus 25 min');
    assert.equal(moment(a).add({ h: 3 }).calendar(), 'आज दोपहर 3:00 बजे', 'Now plus 3 hours');
    assert.equal(moment(a).add({ d: 1 }).calendar(), 'कल दोपहर 12:00 बजे', 'tomorrow at the same time');
    assert.equal(moment(a).subtract({ h: 1 }).calendar(), 'आज दोपहर 11:00 बजे', 'Now minus 1 hour');
    assert.equal(moment(a).subtract({ d: 1 }).calendar(), 'कल दोपहर 12:00 बजे', 'yesterday at the same time');
});

...
test('from', function () {
    ...
    assert.equal(start.from(moment([2007, 1, 28]).add({s: 44}), true),  'ìṣẹ́jú aayá die', '44 seconds = ìṣẹ́jú aayá die');
    assert.equal(start.from(moment([2007, 1, 28]).add({s: 45}), true),  'ìṣẹ́jú kan',      '45 seconds = ìṣẹ́jú kan');
    assert.equal(start.from(moment([2007, 1, 28]).add({s: 89}), true),  'ìṣẹ́jú kan',      '89 seconds = a minute');
    assert.equal(start.from(moment([2007, 1, 28]).add({s: 90}), true),  'ìṣẹ́jú 2',        '90 seconds = ìṣẹ́jú 2');
    assert.equal(start.from(moment([2007, 1, 28]).add({m: 44}), true),  'ìṣẹ́jú 44',       'ìṣẹ́jú 44 = ìṣẹ́jú 44');
    assert.equal(start.from(moment([2007, 1, 28]).add({m: 45}), true),  'wákàtí kan',     'ìṣẹ́jú 45 = wákàtí kan');
    assert.equal(start.from(moment([2007, 1, 28]).add({m: 89}), true),  'wákàtí kan',     'ìṣẹ́jú 89 = wákàtí kan');
    assert.equal(start.from(moment([2007, 1, 28]).add({m: 90}), true),  'wákàtí 2',       'ìṣẹ́jú 90 = wákàtí 2');
    assert.equal(start.from(moment([2007, 1, 28]).add({h: 5}), true),   'wákàtí 5',       'wákàtí 5 = wákàtí 5');
    assert.equal(start.from(moment([2007, 1, 28]).add({h: 21}), true),  'wákàtí 21',      'wákàtí 21 = wákàtí 21');
    assert.equal(start.from(moment([2007, 1, 28]).add({h: 22}), true),  'ọjọ́ kan',        '22 wákàtí = ọjọ́ kan');
    assert.equal(start.from(moment([2007, 1, 28]).add({h: 35}), true),  'ọjọ́ kan',        '35 wákàtí = ọjọ́ kan');
    assert.equal(start.from(moment([2007, 1, 28]).add({h: 36}), true),  'ọjọ́ 2',          'wákàtí 36 = ọjọ́ 2');
...
test('now from now', function () {
    assert.equal(moment().fromNow(), 'ìṣẹ́jú aayá die sẹ́yìn', 'now from now should display as in the past');
});

test('fromNow', function () {
    assert.equal(moment().add({s: 30}).fromNow(), 'ní ìṣẹ́jú aayá die', 'ní ìsẹjú aayá die');

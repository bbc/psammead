...
test('from', function () {
  ...
  assert.equal(start.from(moment([2007, 1, 28]).add({ s: 90 }), true), '2 ਮਿੰਟ', '90 seconds = 2 minutes');
  assert.equal(start.from(moment([2007, 1, 28]).add({ m: 44 }), true), '44 ਮਿੰਟ', '44 minutes = 44 minutes');
  ...
  assert.equal(start.from(moment([2007, 1, 28]).add({ m: 90 }), true), '2 ਘੰਟੇ', '90 minutes = 2 hours');
  assert.equal(start.from(moment([2007, 1, 28]).add({ h: 5 }), true), '5 ਘੰਟੇ', '5 hours = 5 hours');
  assert.equal(start.from(moment([2007, 1, 28]).add({ h: 21 }), true), '21 ਘੰਟੇ', '21 hours = 21 hours');
  ...
  assert.equal(start.from(moment([2007, 1, 28]).add({ h: 36 }), true), '2 ਦਿਨ', '36 hours = 2 days');
  assert.equal(start.from(moment([2007, 1, 28]).add({ d: 1 }), true), 'ਇੱਕ ਦਿਨ', '1 day = a day');
  assert.equal(start.from(moment([2007, 1, 28]).add({ d: 5 }), true), '5 ਦਿਨ', '5 days = 5 days');
  assert.equal(start.from(moment([2007, 1, 28]).add({ d: 25 }), true), '25 ਦਿਨ', '25 days = 25 days');
  ...
  assert.equal(start.from(moment([2007, 1, 28]).add({ d: 46 }), true), '2 ਮਹੀਨੇ', '46 days = 2 months');
  assert.equal(start.from(moment([2007, 1, 28]).add({ d: 74 }), true), '2 ਮਹੀਨੇ', '75 days = 2 months');
  assert.equal(start.from(moment([2007, 1, 28]).add({ d: 76 }), true), '3 ਮਹੀਨੇ', '76 days = 3 months');
  assert.equal(start.from(moment([2007, 1, 28]).add({ M: 1 }), true), 'ਇੱਕ ਮਹੀਨਾ', '1 month = a month');
  assert.equal(start.from(moment([2007, 1, 28]).add({ M: 5 }), true), '5 ਮਹੀਨੇ', '5 months = 5 months');
  assert.equal(start.from(moment([2007, 1, 28]).add({ d: 345 }), true), 'ਇੱਕ ਸਾਲ', '345 days = a year');
  assert.equal(start.from(moment([2007, 1, 28]).add({ d: 548 }), true), '2 ਸਾਲ', '548 days = 2 years');
  assert.equal(start.from(moment([2007, 1, 28]).add({ y: 1 }), true), 'ਇੱਕ ਸਾਲ', '1 year = a year');
  assert.equal(start.from(moment([2007, 1, 28]).add({ y: 5 }), true), '5 ਸਾਲ', '5 years = 5 years');
});
...
test('calendar day', function () {
  ...
  assert.equal(moment(a).calendar(), 'ਅਜ ਦੁਪਹਿਰ 12:00 ਵਜੇ', 'today at the same time');
  assert.equal(moment(a).add({ m: 25 }).calendar(), 'ਅਜ ਦੁਪਹਿਰ 12:25 ਵਜੇ', 'Now plus 25 min');
  assert.equal(moment(a).add({ h: 3 }).calendar(), 'ਅਜ ਦੁਪਹਿਰ 3:00 ਵਜੇ', 'Now plus 3 hours');
  assert.equal(moment(a).add({ d: 1 }).calendar(), 'ਕਲ ਦੁਪਹਿਰ 12:00 ਵਜੇ', 'tomorrow at the same time');
  assert.equal(moment(a).subtract({ h: 1 }).calendar(), 'ਅਜ ਦੁਪਹਿਰ 11:00 ਵਜੇ', 'Now minus 1 hour');
  assert.equal(moment(a).subtract({ d: 1 }).calendar(), 'ਕਲ ਦੁਪਹਿਰ 12:00 ਵਜੇ', 'yesterday at the same time');
});

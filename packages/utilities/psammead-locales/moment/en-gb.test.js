...
test('from', function () {
  ...
  assert.equal(start.from(moment([2007, 1, 28]).add({s: 45}), true),  '1 minute',      '45 seconds = 1 minute');
  assert.equal(start.from(moment([2007, 1, 28]).add({s: 89}), true),  '1 minute',      '89 seconds = 1 minute');
  ...
  assert.equal(start.from(moment([2007, 1, 28]).add({m: 45}), true),  '1 hour',        '45 minutes = 1 hour');
  assert.equal(start.from(moment([2007, 1, 28]).add({m: 89}), true),  '1 hour',        '89 minutes = 1 hour');
...
test('fromNow', function () {
  assert.equal(moment().subtract({m: 1}).fromNow(), '1 minute ago', '1 minute ago');
  assert.equal(moment().subtract({h: 1}).fromNow(), '1 hour ago', '1 hour ago');

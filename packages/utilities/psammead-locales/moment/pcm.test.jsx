...
test('from', function () {
    ...
    assert.equal(start.from(moment([2007, 1, 28]).add({s: 44}), true),  'just some seconds', '44 seconds = a few seconds');
    assert.equal(start.from(moment([2007, 1, 28]).add({s: 45}), true),  'one minute',        '45 seconds = a minute');
    assert.equal(start.from(moment([2007, 1, 28]).add({s: 89}), true),  'one minute',        '89 seconds = a minute');
    ...
    assert.equal(start.from(moment([2007, 1, 28]).add({m: 45}), true),  'one hour',          '45 minutes = an hour');
    assert.equal(start.from(moment([2007, 1, 28]).add({m: 89}), true),  'one hour',          '89 minutes = an hour');
    ...
    assert.equal(start.from(moment([2007, 1, 28]).add({h: 22}), true),  'one day',           '22 hours = a day');
    assert.equal(start.from(moment([2007, 1, 28]).add({h: 35}), true),  'one day',           '35 hours = a day');
    assert.equal(start.from(moment([2007, 1, 28]).add({h: 36}), true),  '2 days',            '36 hours = 2 days');
    assert.equal(start.from(moment([2007, 1, 28]).add({d: 1}), true),   'one day',           '1 day = a day');
    ...
    assert.equal(start.from(moment([2007, 1, 28]).add({d: 26}), true),  'one month',         '26 days = a month');
    assert.equal(start.from(moment([2007, 1, 28]).add({d: 30}), true),  'one month',         '30 days = a month');
    assert.equal(start.from(moment([2007, 1, 28]).add({d: 43}), true),  'one month',         '43 days = a month');
    ...
    assert.equal(start.from(moment([2007, 1, 28]).add({M: 1}), true),   'one month',         '1 month = a month');
    assert.equal(start.from(moment([2007, 1, 28]).add({M: 5}), true),   '5 months',          '5 months = 5 months');
    assert.equal(start.from(moment([2007, 1, 28]).add({d: 345}), true), 'one year',          '345 days = a year');
    assert.equal(start.from(moment([2007, 1, 28]).add({d: 548}), true), '2 years',           '548 days = 2 years');
    assert.equal(start.from(moment([2007, 1, 28]).add({y: 1}), true),   'one year',          '1 year = a year');
...
test('suffix', function () {
    ...
    assert.equal(moment(0).from(30000), 'just some seconds wey don pass', 'suffix');
});

test('now from now', function () {
    assert.equal(moment().fromNow(), 'just some seconds wey don pass',  'now from now should display as in the past');
});

test('fromNow', function () {
    assert.equal(moment().add({s: 30}).fromNow(), 'inside just some seconds', 'in a few seconds');

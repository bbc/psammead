...
test('from', function () {
    ...
    assert.equal(start.from(moment([2007, 1, 28]).add({ s: 45 }), true), '1 мүнөт', '45 seconds = a minute');
    assert.equal(start.from(moment([2007, 1, 28]).add({ s: 89 }), true), '1 мүнөт', '89 seconds = a minute');
    ...
    assert.equal(start.from(moment([2007, 1, 28]).add({ m: 45 }), true), '1 саат', '45 minutes = an hour');
    assert.equal(start.from(moment([2007, 1, 28]).add({ m: 89 }), true), '1 саат', '89 minutes = an hour');
...
test('now from now', function () {
    assert.equal(moment().fromNow(), 'бирнече секунд мурда', 'now from now should display as in the past');
});

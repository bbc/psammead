...
test('format', function () {
    var a = [
        ['dddd, Do MMMM YYYY, a h:mm:ss वाजता', 'रविवार, 14 फेब्रुवारी 2010, दुपारी 3:25:50 वाजता'],
...
test('from', function () {
    ...
    assert.equal(start.from(moment([2007, 1, 28]).add({ d: 5 }), true), '5 दिवस', '5 days = 5 days');
    assert.equal(start.from(moment([2007, 1, 28]).add({ d: 25 }), true), '25 दिवस', '25 days = 25 days');
    ...
    assert.equal(start.from(moment([2007, 1, 28]).add({ d: 46 }), true), '2 महिने', '46 days = 2 months');
    assert.equal(start.from(moment([2007, 1, 28]).add({ d: 74 }), true), '2 महिने', '75 days = 2 months');
    assert.equal(start.from(moment([2007, 1, 28]).add({ d: 76 }), true), '3 महिने', '76 days = 3 months');
    assert.equal(start.from(moment([2007, 1, 28]).add({ M: 1 }), true), 'एक महिना', '1 month = a month');
    assert.equal(start.from(moment([2007, 1, 28]).add({ M: 5 }), true), '5 महिने', '5 months = 5 months');
    assert.equal(start.from(moment([2007, 1, 28]).add({ d: 345 }), true), 'एक वर्ष', '345 days = a year');
    assert.equal(start.from(moment([2007, 1, 28]).add({ d: 548 }), true), '2 वर्षे', '548 days = 2 years');
    assert.equal(start.from(moment([2007, 1, 28]).add({ y: 1 }), true), 'एक वर्ष', '1 year = a year');
    assert.equal(start.from(moment([2007, 1, 28]).add({ y: 5 }), true), '5 वर्षे', '5 years = 5 years');
});

test('suffix', function () {
    assert.equal(moment(30000).from(0), 'काही सेकंदांमध्ये', 'prefix');
    assert.equal(moment(0).from(30000), 'काही सेकंदां पूर्वी', 'suffix');
});

test('now from now', function () {
    assert.equal(moment().fromNow(), 'काही सेकंदां पूर्वी', 'now from now should display as in the past');
});
...
test('calendar day', function () {
    ...
    assert.equal(moment(a).calendar(), 'आज दुपारी 12:00 वाजता', 'today at the same time');
    assert.equal(moment(a).add({ m: 25 }).calendar(), 'आज दुपारी 12:25 वाजता', 'Now plus 25 min');
    assert.equal(moment(a).add({ h: 3 }).calendar(), 'आज दुपारी 3:00 वाजता', 'Now plus 3 hours');
    assert.equal(moment(a).add({ d: 1 }).calendar(), 'उद्या दुपारी 12:00 वाजता', 'tomorrow at the same time');
    assert.equal(moment(a).subtract({ h: 1 }).calendar(), 'आज दुपारी 11:00 वाजता', 'Now minus 1 hour');
    assert.equal(moment(a).subtract({ d: 1 }).calendar(), 'काल दुपारी 12:00 वाजता', 'yesterday at the same time');
});

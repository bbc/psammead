...
test('from', function () {
    ...
    assert.equal(start.from(moment([2007, 1, 28]).add({s: 45}), true), '۱ دقیقه', '45 seconds = a minute');
    assert.equal(start.from(moment([2007, 1, 28]).add({s: 89}), true), '۱ دقیقه', '89 seconds = a minute');
    ...
    assert.equal(start.from(moment([2007, 1, 28]).add({m: 45}), true), '۱ ساعت', '45 minutes = an hour');
    assert.equal(start.from(moment([2007, 1, 28]).add({m: 89}), true), '۱ ساعت', '89 minutes = an hour');

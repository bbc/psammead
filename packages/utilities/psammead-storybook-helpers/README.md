...
`inputProvider` - A function that provides support for previewing components in storybook in different languages. Takes 4 props, `slots` and `componentFunction`, `services` and `options`. Returns the return value of `componentFunction`. This should usually be a React Component.
